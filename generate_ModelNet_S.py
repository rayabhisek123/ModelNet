...
def generate_flexible_similar_subsets_with_visualization(input_dir, 
    ...
    def compute_subset_similarity(subsets):
        all_sims = []
        for class_list in subsets:
            sims = []
    ...
    for subset_idx in range(num_subsets):
        ...
        cluster_pool = [base_cluster] + similar_clusters_map[base_cluster]
        ...
        if len(available_classes) < subset_size:
            print(f"[Skipped] Not enough available classes in cluster pool for subset {subset_idx + 1}")
            continue
        ...
        print(f"✅ Created subset {subset_idx + 1} with {subset_size} similar classes")

    # Plot similarity distribution
    print("🔍 Computing subset similarities...")
    subset_similarities = compute_subset_similarity(subset_class_lists)
    plot_similarity_histogram(subset_similarities, save_path)

    # Save results
    sim_df = pd.DataFrame({
        "subset_id": list(range(1, len(subset_similarities) + 1)),
    ...
    subset_class_map = {
        f"subset_{i + 1}": subset_class_lists[i]
        for i in range(len(subset_class_lists))
    }
...
if __name__ == "__main__":
    ...
    output_dir = "/home/ar/FLOCKD/NN_Classification/train_1-100_5000-15_cifar100_similar_mod"
    save_path = "/home/ar/FLOCKD/NN_Classification/results/data_split_train_1-100_5000-15_similar_mod"

    class_embeddings = get_class_representations(input_dir, max_images_per_class=75)
